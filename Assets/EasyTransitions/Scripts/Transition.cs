...
namespace EasyTransition
{
    ...
    public class Transition : MonoBehaviour
    {
        ...
        {
            if (hasTransitionTriggeredOnce) return; // Prevent multiple fade-outs
            ...
            if (!transitionSettings.isCutoutTransition)
            {
                if (transitionOut.TryGetComponent<Image>(out Image parentImage))
                {
                    parentImage.material = transitionSettings.colorTintMode == ColorTintMode.Multiply ? multiplyColorMaterial : additiveColorMaterial;
                    parentImage.material.SetColor("_Color", transitionSettings.colorTint);
                }
                foreach (Transform child in transitionOut.transform)
                {
                    if (child.TryGetComponent<Image>(out Image childImage))
                    {
                        childImage.material = transitionSettings.colorTintMode == ColorTintMode.Multiply ? multiplyColorMaterial : additiveColorMaterial;
                        childImage.material.SetColor("_Color", transitionSettings.colorTint);
                    }
                }
            }

            // Flip scale if needed
            if (transitionSettings.flipX)
                transitionOut.transform.localScale = new Vector3(-transitionOut.transform.localScale.x, transitionOut.transform.localScale.y, transitionOut.transform.localScale.z);
            if (transitionSettings.flipY)
                transitionOut.transform.localScale = new Vector3(transitionOut.transform.localScale.x, -transitionOut.transform.localScale.y, transitionOut.transform.localScale.z);
            ...
            else
            {
                foreach (Transform child in transitionOut.transform)
                {
                    if (child.TryGetComponent<Animator>(out Animator childAnim) && transitionSettings.transitionSpeed != 0)
                        childAnim.speed = transitionSettings.transitionSpeed;
                }
            }
